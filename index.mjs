//  ====================================================
//  SQL SETUP (run these once in Railway's query tab)
//  ====================================================
//  CREATE TABLE users (
...
//  );
//
//  INSERT INTO seats (isbooked, movie_id) SELECT 0, 1 FROM generate_series(1, 20);
...
// ✅ FIX: Railway internal host (postgres.railway.internal) does NOT use SSL.
// SSL is only needed when connecting from outside Railway (e.g. your laptop via public URL).
// Using ssl:true on an internal connection causes "unexpected eof" crash.
const pool = new pg.Pool({
  connectionString: process.env.DATABASE_URL,
  ssl: process.env.DATABASE_URL && process.env.DATABASE_URL.includes("railway.internal")
    ? false                              // internal Railway → no SSL needed
    : { rejectUnauthorized: false },     // external connection → SSL without cert verify
});
...
// ── Movies (assumption/placeholder data — not from DB) ───────────────────────
// NOTE: This is mock data. To use real DB data, create a movies table instead.
const movies = [
...
// Legacy endpoint — kept so nothing breaks
...
app.get("/movies",     (req, res) => res.sendFile(__dirname + "/movies.html"));
app.get("/seats-page", (req, res) => res.sendFile(__dirname + "/seats.html"));
app.get("/",           (req, res) => res.sendFile(__dirname + "/movies.html"));
